...
deb http://wawadeb.crdp.ac-caen.fr/debian jessiebackports smb41
END
}
#date
LADATE=$(date +%d-%m-%Y)
chemin_migr="/root/migration_wheezy2jessie"
mkdir -p $chemin_migr
...
echo -e "$COLTITRE"
echo "*********************************************"
echo "* Script de migration de Wheezy vers Jessie *" | tee -a $fichier_log
echo "*********************************************"
echo -e "$COLTXT"
}
...
if [ "$NODL" != "yes" ]; then
	...
	echo -e "$COLTXT"
	cd /root
	SCRIPT_FILE="se3_upgrade_jessie.sh"
	SCRIPT_FILE_MD5="se3_upgrade_jessie.md5"
...
# 	tar -xzf $SCRIPT_FILE
	...
	cd -
# 	MD5_CTRL1=$(cat se3_upgrade_jessie.md5)
	...
	then
		echo -e "$COLINFO"
		echo "Les scripts de migration ont ete mis a jour depuis le serveur central, veuiller relancer se3_upgrade_jessie.sh"
...
if [ ! -e $chemin_migr/phase2b-ok ]; then
	...
	# Creation du source.list jessie
...
echo -e "$COLPARTIE"
echo "Partie 6 : Mise a jour des paquets se3 sous jessie"  | tee -a $fichier_log
echo -e "$COLTXT"
