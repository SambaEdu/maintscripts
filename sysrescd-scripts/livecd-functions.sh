...
livecd_get_cmdline() {
	...
	CMDLINE="$(cat /proc/cmdline) cdroot"
	export CMDLINE
}
...
livecd_config_ip() {
	...
	dialog --visit-items --title "Configuration TCP/IP" --menu "Vous pouvez utiliser DHCP pour configurer automatiquement le réseau, ou spécifier une IP, masque,... manuellement.\nChoisissez une option:" 20 60 7 1 "Utiliser DHCP pour auto-détecter mes paramètres" 2 "Spécifier une IP manuellement" 2> ${iface}.DHCP
	...
	case ${DHCP} in
		...
		2)
			...
			dialog --visit-items --title "Adresse IP" --inputbox "Veuillez entrer une adresse IP pour ${iface}:" 20 50 "192.168.1.1" 2> ${iface}.IP
			...
			BC_TEMP=$IP1.$IP2.$IP3.255

			#dialog --visit-items --title "Broadcast address" --inputbox "Please enter a Broadcast address for ${iface}:" 20 50 "${BC_TEMP}" 2> ${iface}.BC
			dialog --visit-items --title "Adresse de Broadcast" --inputbox "Veuillez entrer une adresse de Broadcast pour ${iface}:" 20 50 "${BC_TEMP}" 2> ${iface}.BC
			...
			dialog --visit-items --title "Masque de sous-réseau" --inputbox "Veuillez entrer le masque de sous-réseau pour ${iface}:" 20 50 "255.255.255.0" 2> ${iface}.NM
			...
			dialog --visit-items --title "Passerelle" --inputbox "Veuillez entrer une adresse de passerelle pour ${iface} (taper ENTREE pour aucune:)" 20 50 2> ${iface}.GW
			...
			dialog --visit-items --title "Serveur DNS" --inputbox "Veuillez entrer une adresse de serveur DNS (taper ENTREE pour aucun:)" 20 50 2> ${iface}.DNS
