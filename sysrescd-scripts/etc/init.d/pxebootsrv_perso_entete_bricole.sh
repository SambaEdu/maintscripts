...
start()
{
	...
	then
		...
		then
			...
			sed -i -e "s!scandelay=.!scandelay=5 netboot=$PXEBOOTSRV_HTTPSERVER ${PXEBOOTSRV_EXTRA} autoruns=1 ar_source=http://$PXEBOOTSRV_HTTPSERVER/ ar_nowait!g" /tftpboot/pxelinux.cfg/default
...
kernel rescue32
initrd initram.igz
append scandelay=5 netboot=$PXEBOOTSRV_HTTPSERVER setkmap=fr work=cli_partimaged.sh ip_serveur_partimaged=$PXEBOOTSRV_LOCALIP autoruns=1 ar_source=http://$PXEBOOTSRV_HTTPSERVER/ ar_nowait
" >> /tftpboot/pxelinux.cfg/default
			#=================================
